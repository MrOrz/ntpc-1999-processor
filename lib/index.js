...
var processedRows = [];
var processStreams = IN_FILES.map(infile => fs.createReadStream(infile).pipe(progressFactory(infile)).pipe(iconv.decodeStream('big5')).pipe(parserFactory()));
...
MultiStream(processStreams).on('close', () => {
